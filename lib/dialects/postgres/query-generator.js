  ...
  var QueryGenerator = {
    ...
    selectQuery: function(tableName, options) {
      ...
      if (options.include) {
        ...
        options.include.forEach(function(include) {
          var inclAttrs = Object.keys(include.daoFactory.attributes)
          if(include.attributes) {
            inclAttrs = include.attributes 
          }

          var attributes = inclAttrs.map(function(attr) {
          ...
          query += Utils._.template(joinQuery)({
            table:      QueryGenerator.addQuotes(include.daoFactory.tableName),
            as:         include.as,
            tableLeft:  ((include.association.associationType === 'BelongsTo') ? include.as : QueryGenerator.addQuotes(tableName)),
            attrLeft:   'id',
            tableRight: ((include.association.associationType === 'BelongsTo') ? QueryGenerator.addQuotes(tableName) : include.as),
      ...
      if(options.order) {
        var newOrder = []
        Utils._.each(options.order.split(','), function(f) {
          var newField = ''
          newOrder.push(f.replace(/([^ ]+)(.*)/, function(m, g1, g2) {
            return QueryGenerator.addQuotes(g1) + g2
          }))
        })
        options.order = newOrder.join(',')
    ...
    insertQuery: function(tableName, attrValueHash) {
      ...
      Utils._.forEach(attrValueHash, function(value, key, hash) {
        ...
        // rraux - added to allow ability to turn omitNull to false.
