...
module.exports = (function() {
  var ConnectorManager = function(sequelize, config) {
    ...
    // WTM - we are currently modifying the types to get nums rather than strings
    ...
    /**
    bigint (20), postgres bigint can not be converted to a number in javascript
    **/
    this.pg.types.setTypeParser(20, 'text', function(str) {  
      if(str.length > 20) {
        throw new Error('Cannot parse ' + str + ' to an integer. Sorry about JavaScripts lack of 64bit numbers')
      }
      return parseInt(str, 10)
    })

    this.pg.types.setTypeParser(1082, 'text', function(val) { return String(val) })
  ...
  ConnectorManager.prototype.connect = function(callback) {
    ...
    if (this.pooling) {
      ...
      //wtm - using md5 this is currently the only way
      //we are getting connections
      this.sequelize.config["user"] = this.sequelize.config.username
