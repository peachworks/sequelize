...
module.exports = (function() {
  var QueryGenerator = {
    ...
    */
    ...
    //wtm
    disableQuotes: function() {
      throwMethodUndefined('disableQuotes')
    },

    //wtm
    enableQuotes: function() {
      throwMethodUndefined('enableQuotes')
    },
    ...
    */
    ...
    selectQuery: function(tableName, options, factory) {
        ...
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (options.includeIgnoreAttributes !== false) {
            attributes  = include.attributes.map(function(attr) {
              ...
              return self.quoteIdentifier(as) + "." + self.quoteIdentifier(attr) + " AS " + self.quoteIdentifier(as + "." + attr, true)
            })
          ...
          // {{{
          //wtm - NOTE: we did not modify any "through" code below to force quotes as we currently don't do any "through"
          //type calls in the api, if we need to this is where we would need to make changes.  This not is also
          //to server whoever may have to merge/purge sequelize in the future
          if (through) {
            ...
            if (subQuery && !include.subQuery && include.parent.subQuery) {
              ...
              if(subQuery) {
                //wtm - force quote the "as"
                where = self.quoteIdentifier(tableLeft, true) + "." + self.quoteIdentifier(attrLeft) + " = "
              } else {
            ...
            //wtm -quoting
            if(subQuery) {
              where += self.quoteIdentifier(tableRight, true) + "." + self.quoteIdentifier(attrRight)
            } else {
            ...
            // Generate join SQL
            //wtm - force quote the "as"
            joinQueryItem += joinType + self.quoteIdentifier(table) + " AS " + self.quoteIdentifier(as, true) + " ON "
              joinQueryItem += where
      ...
      if (subQuery) {
        ...
        //if we find options.table to exactly match the first item in mainAttributes quote it

        //wtm - the subquery "AS" uses the tableName at the front of the SELECT (out of the subquery)
        //and as the "AS", when this is the case, force quoting on the mainAttributes "options.table"
    ...
    getWhereConditions: function(smth, tableName, factory, options, prepend) {
      ...
      if ((smth instanceof Utils.and) || (smth instanceof Utils.or)) {
        ...
        if (treatAsAnd) {
          ...
          //wtm - ARRAY constructors in where conditions don't work
          if(this.dialect === "postgres") {
            result = SqlString.pgArrayFormat(smth.shift(), smth)
          } else {
