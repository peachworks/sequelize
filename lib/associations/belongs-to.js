...
module.exports = (function() {
  ...
  BelongsTo.prototype.injectAttributes = function() {
    ...
    //wtm - in 1.7.0 not sure if this is still necessary but we needed it prior to this
    if(!!this.options.allowNull) {
      this.source.rawAttributes[this.identifier]['allowNull'] = this.options.allowNull
    }

    //wtm - keyValidator is our own piece for doing include validation
    if(!!this.options.keyValidator) {
      this.source.DAO.prototype.setValidators(this.identifier, this.options.keyValidator)
    }
