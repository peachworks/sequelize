var moment = require("moment")
  , isArrayBufferView
  , SqlString = exports
  , isPgWhereArray = false; // wtm - fix array in queries
...
SqlString.arrayToList = function(array, timeZone, dialect, field) {
  if (dialect === 'postgres' && !isPgWhereArray) { // - wtm fix pg in queries
    if (array.map) {
...
//wtm - fix pg in queries
SqlString.pgArrayFormat = function(sql, values) {
  values = [].concat(values);
  ...
  return sql.replace(/\?/g, function(match) {
    ...
    isPgWhereArray = true;
    var ret = SqlString.escape(values.shift(), false, null, 'postgres');
    isPgWhereArray = false;
    return ret
  });
};
