  ...
  DAOFactory.prototype.init = function(daoFactoryManager) {
    ...
    Utils._.each(this.rawAttributes, function(columnValues, columnName) {
      ...
      if(columnValues.primaryKey){
        self.primaryKeys[columnName] = self.attributes[columnName]
      }
      if(columnValues.autoIncrement)
      {
        self.autoIncrementField = columnName
      }
      if (((columnValues === DataTypes.BOOLEAN) || (columnValues.type === DataTypes.BOOLEAN))) {
        self.booleanValues.push(columnName);
      }
      if (((columnValues === DataTypes.DATE) || (columnValues.type === DataTypes.DATE) || (columnValues.originalType === DataTypes.DATE))) {
        self.dateAttributes.push(columnName);
      }
      if (columnValues.hasOwnProperty('defaultValue')) {
        self.defaultValues[columnName] = Utils._.partial(
          Utils.toDefaultValue, columnValues.defaultValue)
      }

      if (columnValues.hasOwnProperty('validate')) {
        self.validators[columnName] = columnValues.validate;
      }
    })
    ...
    if (this._timestampAttributes.createdAt
      && !Utils._.contains(self.dateAttributes,this._timestampAttributes.createdAt)) {
      self.dateAttributes.push(this._timestampAttributes.createdAt)
    }
    if (this._timestampAttributes.updatedAt
      && !Utils._.contains(self.dateAttributes,this._timestampAttributes.updatedAt)) {
      self.dateAttributes.push(this._timestampAttributes.updatedAt)
    }
    if (this._timestampAttributes.deletedAt
      && !Utils._.contains(self.dateAttributes,this._timestampAttributes.deletedAt)) {
      self.dateAttributes.push(this._timestampAttributes.deletedAt)
    }
    ...
    this.DAO.prototype.booleanValues = self.booleanValues
    this.DAO.prototype.dateAttributes = self.dateAttributes
    this.DAO.prototype.defaultValues = self.defaultValues
    this.DAO.prototype.validators    = self.validators
  ...
  */
  ...
  DAOFactory.prototype.findAndCountAll = function(findOptions, queryOptions) {
    var self  = this
      ...
      , countOptions = Utils._.omit(findOptions ? Utils._.merge({}, findOptions) : {}, ['include', 'offset', 'limit', 'order', 'attributes'])
  ...
  var addDefaultAttributes = function() {
    ...
    self.rawAttributes = Utils._.extend(attributes,self.rawAttributes)
    if (this._timestampAttributes.createdAt) {
      self.rawAttributes[this._timestampAttributes.createdAt] = {type: DataTypes.DATE, allowNull: false}
    }
    if (this._timestampAttributes.updatedAt) {
      self.rawAttributes[this._timestampAttributes.updatedAt] = {type: DataTypes.DATE, allowNull: false}
    }
    if (this._timestampAttributes.deletedAt) {
      self.rawAttributes[this._timestampAttributes.deletedAt] = {type: DataTypes.DATE}
    }
  }
